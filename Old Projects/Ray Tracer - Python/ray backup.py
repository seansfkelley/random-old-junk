...
BKRD_COLOR=(1,1,1)
AMBIENCE=.3
CAMERA=vector(1000,1000,1000)
FOCUS_POINT=vector(0,0,0)
CANVAS=(512,512)
RESOLUTION=4.0#pixels per real world units, or scale/zoom. must be float
...
class RectPrism:
    ...
    def collide(self, ray, tMax=0):
        ...
        if values and values[2]==5:
            print 'hit topside on',self,'at',self.position
        return values
    def findColor(self, ray, t, otherInfo):
            ...
            try:
                ...
                if choice==-1:#yes
                    writeFile(bytes, t, y+1, 'User stopped after %d pixels, %.2f%% completed.'\
                              % (progress.curval, float(progress.curval)/TOTAL_AREA))#y is one less than the equivalent line number
                    exit()
...
def generateHills():
    ...
    randPercentSet=lambda: (randint(0,100)/100.0,randint(0,100)/100.0,randint(0,100)/100.0)
    ...
    for i in range(ITERATIONS):
        ...
        for x2 in range(X_SIZE):
            for z2 in range(Z_SIZE):
                ...
                if y>0:
                    terrain[x2][z2]+=y
                    if terrain[x2][z2]>maximum:
                        maximum=terrain[x2][z2]
                    elif terrain[x2][z2]<minimum:
                        minimum=terrain[x2][z2]
        try:
            progress.inc()
...
##    exit()
    ...
    for x in range(X_SIZE):
        for z in range(Z_SIZE):
            y=(((terrain[x][z]-minimum)/(maximum-minimum))**2)*HEIGHT
            x1=x-X_SIZE/2
            z1=z-Z_SIZE/2
            g_shapes.append(RectPrism((x1*TER_BOX_SIZE,y/2,z1*TER_BOX_SIZE), (TER_BOX_SIZE, y, TER_BOX_SIZE), randPercentSet()))
...
##        a=not a
##        while x<=80:
##            g_shapes.append(RectPrism((x,0,z),(40,40,40),color))
##            x+=80
##        z+=40
...
##        a=not a
##        while x<=80:
##            g_shapes.append(RectPrism((x,-40,z),(40,40,40),randPercentSet()))
##            x+=80
##        z+=40

##    g_shapes.append(RectPrism((0,0,0),(96,96,96),(.5,.5,.5)))
##    g_shapes.append(RectPrism((-512,0,0), (96,96,96), (.5,.5,.5)))
...
##    for i in range(20):
##        g_shapes.append(RectPrism(randIntSet(-100,100), (32,32,32), randPercentSet(), randint(25,75)/100.0, 0))
