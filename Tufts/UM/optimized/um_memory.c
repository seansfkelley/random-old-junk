...
UM_word UM_memory_get(T memory, UM_word array_i, UM_word element_i){
  return ((UM_word *)Seq_get(memory->c_arrays, array_i))[element_i];
}

void UM_memory_set(T memory, UM_word array_i, 
		   UM_word element_i, UM_word value){
  ((UM_word *)Seq_get(memory->c_arrays, array_i))[element_i] = value;
}
...
UM_word UM_memory_activate(T memory, UM_word size){
  ...
  UM_word *array = NULL;
  if (size > 0){
    array = CALLOC(size,  sizeof(UM_word));
    memset(array, (UM_word) 0, size * sizeof(UM_word));
  }
...
void UM_memory_load_program(T memory, UM_word inst_array_i){
  ...
  array = CALLOC(num_words, sizeof(UM_word));
