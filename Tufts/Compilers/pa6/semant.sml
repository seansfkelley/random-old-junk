...
struct
    ...
    and transFunDecs (level, venv, tenv, {name, params, result = SOME(id, _), body, pos} :: fdecs, break) = 
        let 
            ...
            fun enterparam({name, ty}::ps, venv, i) = enterparam(ps, S.enter(venv, name, Env.VarEntry{access = Translate.allocParam(level', i)(true), ty = ty}), i+1)
              | enterparam([], venv, _)          = venv
            val venv'' = enterparam(params', venv', 0)
            ...
            fun trexp (A.VarExp(var)) = trvar var
              ...
              | trexp (A.LetExp{decs, body, pos}) =
                  let 
                      ...
                      val body' = transExp(level, venv', tenv', break) body
                  in
                      {exp = Trans.letExp(rev inits', #exp body'), ty = #ty body'}
                  end
    ...
    fun transProg e = (Trans.unNx (#exp (transExp(Translate.newLevel{parent = Translate.outermost, name = Temp.newlabel(), formals = []}, value_env, type_env, Temp.newlabel()) e)))
