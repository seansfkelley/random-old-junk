...
public class GraphView extends JPanel {
    ...
    public GraphView(Graph g, String label) {
        ...
        ActionList printFields = new ActionList();
        printFields.add(new PrintAction(m_vis));
        m_vis.putAction("print", printFields);
        ...
        display.addControlListener(new FocusControl(1, "print"));
...
//        opanel.add(overview);
    ...
    public static class PrintAction extends Action {
    	...
    	public PrintAction(Visualization v){
    		m_vis = v;
    	}
    	
    	public void run(double frac){
    		for (Iterator iter = m_vis.getGroup(Visualization.FOCUS_ITEMS).tuples(); iter.hasNext();){
    			Tuple t = ((VisualItem) iter.next()).getSourceTuple();
    			for (int i = 0; i < t.getColumnCount(); ++i){
    				if (t.get(i) != null)
    					System.out.println(t.getColumnName(i) + " " + t.get(i).toString());
    			}
    		}
    	}
    }
